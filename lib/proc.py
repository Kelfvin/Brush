...
def proc(base_path, subject_name, out_file_name):
    ...
    for i in range(0, 100):
        try:
            file_in = open(f"{base_path}d{i}" + ".in", encoding="utf-8", mode="rt")
    ...
    for i in range(0, 100):
        try:
            ...
            for index in range(0, len(titles)):
                ...
                ProblemItem["options"] = re.findall(
                    r"[A-Z]、\n*?.+?\n", optionsList[index]
                )
...
def proc_Marxism(base_path, subject_name, out_file_name):
    ...
    for i in nameList:
        try:
            file_in = open(f"{base_path}{i}" + ".txt", encoding="utf-8", mode="rt")
